...
Convert::Convert(ros::NodeHandle node, ros::NodeHandle private_nh) : data_(new lslidar_rawdata::RawData()){
    ...
    //~ in `processScan, publish the sweep (by `publishScan); TransportHints() is used for transport layer.
    packet_sub_ = node.subscribe(input_packets_topic, 10, &Convert::processScan, (Convert *)this, ros::TransportHints().tcpNoDelay(true));
    ...
    sweep_data = lslidar_c32_msgs::LslidarC32SweepPtr(new lslidar_c32_msgs::LslidarC32Sweep());
}
